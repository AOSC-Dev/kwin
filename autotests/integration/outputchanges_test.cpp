...
void OutputChangesTest::testInvalidGeometryRestoreAfterEnablingOutput()
{
    ...
    QCOMPARE(window->geometryRestore().size(), QSizeF());
    const QRectF previousGeometryRestore = window->geometryRestore();
    ...
    QCOMPARE(window->maximizeMode(), MaximizeFull);
    QCOMPARE(window->requestedMaximizeMode(), MaximizeFull);
    QCOMPARE(window->geometryRestore(), previousGeometryRestore);
}
