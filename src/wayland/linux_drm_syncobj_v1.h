...
class LinuxDrmSyncObjV1Interface : public QObject, private QtWaylandServer::wp_linux_drm_syncobj_v1
{
public:
...
private:
    void wp_linux_drm_syncobj_v1_get_surface(Resource *resource, uint32_t id, wl_resource *surface) override;
    void wp_linux_drm_syncobj_v1_import_timeline(Resource *resource, uint32_t id, int32_t fd) override;
    void wp_linux_drm_syncobj_v1_destroy(Resource *resource) override;
...
private:
    const std::unique_ptr<SyncTimeline> m_timeline;
};
