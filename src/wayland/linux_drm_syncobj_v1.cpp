...
LinuxDrmSyncObjV1Interface::LinuxDrmSyncObjV1Interface(Display *display, QObject *parent)
    : QObject(parent)
    , QtWaylandServer::wp_linux_drm_syncobj_v1(*display, s_version)
{
}

void LinuxDrmSyncObjV1Interface::wp_linux_drm_syncobj_v1_get_surface(Resource *resource, uint32_t id, wl_resource *surface)
{
...
void LinuxDrmSyncObjV1Interface::wp_linux_drm_syncobj_v1_import_timeline(Resource *resource, uint32_t id, int32_t fd)
{
    ...
    if (!timeline) {
        wl_resource_post_error(resource->handle, WP_LINUX_DRM_SYNCOBJ_V1_ERROR_INVALID_TIMELINE, "Importing timeline failed");
        return;
    }
...
{
    QtWaylandServer::wp_linux_drm_syncobj_v1::globalRemove();
}

void LinuxDrmSyncObjV1Interface::wp_linux_drm_syncobj_v1_destroy(Resource *resource)
{
...
SyncTimeline *LinuxDrmSyncObjTimelineV1::timeline() const
{
...
void LinuxDrmSyncObjSurfaceV1::wp_linux_drm_syncobj_surface_v1_set_acquire_point(Resource *resource, wl_resource *timeline_resource, uint32_t point_hi, uint32_t point_lo)
{
    ...
    SurfaceInterfacePrivate::get(m_surface)->pending->explicitSyncFd = timeline->timeline()->eventFd(point);
}
