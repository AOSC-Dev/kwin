...
std::unique_ptr<IccProfile> IccProfile::load(const QString &path)
{
    ...
    if (cmsIsTag(handle, cmsSigBToA1Tag)) {
        ...
        if (data) {
            return std::make_unique<IccProfile>(handle, Colorimetry::fromXYZ(red, green, blue, white, black), std::move(*data), vcgt);
        } else {
    ...
    return std::make_unique<IccProfile>(handle, Colorimetry::fromXYZ(red, green, blue, white, black), inverseEOTF, vcgt);
}
